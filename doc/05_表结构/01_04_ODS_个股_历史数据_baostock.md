...
|           是否新增           |      参数名称      |              参数描述              |  示例数据  |   单位   |                说明                 |                             备注                             |
| :--------------------------: | :----------------: | :--------------------------------: | :--------: | :------: | :---------------------------------: | :----------------------------------------------------------: |
|              1               |    primary_key     |                主键                |            |          |                                     | 1.确定唯一数据，md5（日期、时间、代码）<br />2.关联关联信息  |
|                              |        date        |           交易所行情日期           | 2017-07-03 |          |          格式：YYYY-MM-DD           |                                                              |
|                              |        code        |              证券代码              | sh.600000  |          | 格式：sh.600000。sh：上海，sz：深圳 |                                                              |
|                              |        open        |               开盘价               |   12.64    |          |  精度：小数点后4位；单位：人民币元  |                                                              |
...
| prev_close（previous close） |      preclose      |             昨日收盘价             |   12.65    | 人民币元 |          精度：小数点后4位          | 证券在指定交易日行情数据的前收盘价，当日发生除权除息时，“前收盘价”不是前一天的实际收盘价，而是根据股权登记日收盘价与分红现金的数量、配送股的数里和配股价的高低等结合起来算出来的价格。 |
|                              |       volume       |              成交数量              |  38778949  |    股    |                                     |                                                              |
|                              |       amount       |              成交金额              | 486264672  | 人民币元 |          精度：小数点后4位          |                                                              |
|                              |     adjustflag     |              复权状态              |     3      |          |       不复权、前复权、后复权        |                                                              |
|                              |        turn        |               换手率               |  0.137985  |    %     |          精度：小数点后6位          |                                                              |
|                              |    tradestatus     |              交易状态              |     1      |          |         1：正常交易 0：停牌         |                                                              |
|      Percentage Change       |       pctChg       |          涨跌幅（百分比）          | —0.711456  |          |          精度：小数点后6位          |      涨跌幅=(当日收盘价 - 昨日收盘价 / 昨日收盘价)×100%      |
