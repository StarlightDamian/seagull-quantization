...
from test_ import test_2_price_limit, test_2_recommend, test_2_stock_pick, test_2_short_term_recommend


class lightgbmTestMain:
    def __init__(self):
        self.test_price_limit = test_2_price_limit.testPriceLimit()
        self.test_stock_pick = test_2_stock_pick.testStockPick()
        self.test_stock_recommend = test_2_recommend.testStockRecommend()
        self.test_short_term_recommend = test_2_short_term_recommend.testShortTermRecommend()
    ...
    def stock_pick_pipline(self, history_day_df):
        ...
        # 一字涨跌停
        price_limit_df = self.test_price_limit.test_pipline(history_day_df)
        #price_limit_df1 = price_limit_df
        price_limit_df = price_limit_df[['primary_key', 'rear_price_limit_pred']]
        
        # 日内上升
        stock_recommended_df = self.test_stock_recommend.test_pipline(history_day_df)
        #stock_recommended_df1 =stock_recommended_df
        stock_recommended_df = stock_recommended_df[['primary_key', 'rear_rise_pct_pred']]

        # 选股
        stock_pick_df = self.test_stock_pick.test_pipline(history_day_df)
        ...
        short_term_recommend_df = self.test_short_term_recommend.test_pipline(history_day_df)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--date_start', type=str, default='2024-02-20', help='Start time for backtesting')
    #parser.add_argument('--date_end', type=str, default='2022-01-01', help='End time for backtesting')
    parser.add_argument('--date_end', type=str, default='2024-03-13', help='End time for backtesting')
    ...
    lightgbm_test_main = lightgbmTestMain()
    prediction_df = lightgbm_test_main.stock_pick_pipline(history_day_df)
