# -*- coding: utf-8 -*-
"""
Created on Mon Dec  9 23:15:48 2024
...
class AutoEncoder(nn.Module):
    def __init__(self, num_features, hidden_units, dropout_rates):
        ...
        # Decoder (symmetrical structure)
        self.decoder_layers = nn.ModuleList([nn.Linear(hidden_units[i], hidden_units[i-1]) for i in range(len(hidden_units)-1, 0, -1)])
        self.output_layer = nn.Linear(hidden_units[0], num_features)  # Output layer to match input features size
        
        # For classification (multi-label)
        self.classification_layer = nn.Linear(hidden_units[-1], 1)  # Adjust for classification
    ...
    def forward(self, x):
        ...
        # Decoder
        for i in range(0, len(self.decoder_layers)):
            x = self.decoder_layers[i](x)
            x = torch.nn.functional.silu(x)  # Swish activation
        
        # Final output for reconstruction
        output_reconstruction = self.output_layer(x)
        
        # Classification
        classification_output = self.classification_layer(x)
        classification_output = self.sigmoid(classification_output)  # Apply sigmoid to the classification output

        return output_reconstruction, classification_output
...
for i in range(num_models):
    model = AutoEncoder(num_features=len(features), hidden_units=hidden_units, dropout_rates=dropout_rates)
    # model.load_state_dict(torch.load(f'../input/js-nn-models/JSModel_{i}.pth'))  # Load model weights
    model.eval()  # Set to evaluation mode
    models.append(model)

# Assuming f_mean is already loaded or manually defined (if you don't have access to it)
f_mean = np.random.randn(1, len(features))  # Replace with actual f_mean or generate a placeholder

# Mockup test data for testing purposes (replace with actual data)
...
for idx, test_df in tqdm(test_data.iterrows(), total=len(test_data)):
    if test_df['weight'] > 0:
        ...
        pred_classification = 0.0
        
        # Make predictions with the models
        for model in models:
            with torch.no_grad():
                _, pred_cls = model(x_tt_tensor)  # Get classification output
                pred_classification += pred_cls.item() / len(models)
        
        # Apply threshold to make a decision for classification
        action = 1 if pred_classification >= opt_th else 0
