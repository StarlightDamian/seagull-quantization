...
class DwdStock(ods_ohlc_incr_baostock_stock_sh_sz_api.OdsIncrBaostockStockShSzApi):#OdsFreqShSzStock
    ...
    def stock_daily_full(self):
        ...
        # asset_daily_df.loc[zero_volume, 'vwap'] = np.nan
        # asset_daily_df.loc[~zero_volume,'vwap'] = asset_daily_df.loc[~zero_volume, 'turnover'] / asset_daily_df.loc[~zero_volume, 'volume']#每单位成交量的平均价格
        # asset_daily_df['vwap'] = asset_daily_df['vwap'].round(4)
...
# =============================================================================
    ...
    def stock_daily_incr(self, df, date_start):
        ...
        with utils_database.engine_conn("POSTGRES") as conn:
            ...
            efinance_portfolio_daily_df['market_code'] = efinance_portfolio_daily_df['股票代码'].map(portfolio_market_dict)
            efinance_portfolio_daily_df[['board_type', 'price_limit_rate']] = 'ETF', 0.1
            efinance_portfolio_daily_df.loc[
                efinance_portfolio_daily_df['股票名称'].str.contains('科创|创业|双创'), 'price_limit_rate'] = 0.2
        ...
        # 数据来源表
        clean_stock_sh_sz_df['source_table'] = 'ods_ohlc_stock_incr_baostock_sh_sz_daily'
        clean_stock_bj_df['source_table'] = 'ods_ohlc_stock_incr_efinance_bj_daily'
        clean_portfolio_df['source_table'] = 'ods_ohlc_fund_incr_efinance_daily'
        ...
        print('3', asset_daily_df.shape)
        #return asset_daily_df, clean_stock_sh_sz_df, clean_stock_bj_df, clean_portfolio_df
        #asset_daily_df['vwap'] = asset_daily_df['turnover'] / asset_daily_df['volume']#每单位成交量的平均价格
        #asset_daily_df['vwap'] = asset_daily_df['vwap'].round(4)
